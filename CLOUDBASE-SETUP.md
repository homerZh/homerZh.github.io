...
2026-09-25 已在现有环境执行 `cloudbase/blood-pressure.sql`，创建 `blood_pressure_records`。仅既有管理员用户可读取、插入；没有开放匿名读取。`blood-pressure.html` 可选择保存时的当前时间或补录过去的北京时间，填写上压、下压、心跳。历史区查询测量时间最新的 50 条；查询数量限制不删除数据库中更早的记录。该页面仅用于记录，不提供医疗判断。

## 饮食热量功能

2026-09-25 已执行 `cloudbase/calorie-entries.sql`：加餐和运动改为逐条记录，可写备注、热量并单独修改或删除。此前按天保存的 4 条记录已迁入新表，显示为“此前记录的当日总量”。顶部查看日期统一控制三餐与加餐、运动列表。旧表中的加餐、运动行仍保留以便核对，但新版统计只读取新表，避免重复计算。

在 SQL 编辑器执行 `cloudbase/calories.sql` 一次，新增计划和三餐记录表。原测试表保持不变。
打开 `/calories.html`，使用 homer 登录。每个日期、每个餐次只有一条记录，重复保存更新该条记录；清除后差额为 0。
计划为六个日历月，结束日期不计入累计范围，按上海日期记录。
每日 2200 千卡仅为可调整的激励基准（早餐 550 / 中餐 880 / 晚餐 770），不是按个人生理参数计算的精确需求。
目标采用固定换算 15 × 7700 = 115500 千卡。只显示相对饮食基准的差额，不推断真实体重变化。
已有记录保存基准快照，调整基准不会追溯修改历史记录。

部署前验证：保存早餐 400（基准 550）应少 150；中餐 1000（基准 880）应多 120；晚餐不记录，当天净减少为 30。刷新后应保持；修改、清除及未授权访问需在真实环境继续验证。

...
export const ACTIVITY_TYPES = ['snack', 'exercise'];
...
export function difference(record) {
  if (!record) return 0;
  const type = record.meal ?? record.kind;
  return type === 'exercise' ? record.calories : type === 'snack' ? -record.calories : record.baseline-record.calories;
}
